...
def Score(ticker: str) -> int:
    ...
    if L < One_Year_Ish:
        ...
        _52_week_low = min(closing_price[-One_Year_Ish::])
        _52_week_high = max(closing_price[-One_Year_Ish::])
    ...
    if (price_uptodate > rolling_averages[150]) and (
        price_uptodate > rolling_averages[200]
    ):
        score += 1
    ...
    if (rolling_averages[50] > rolling_averages[200]) and (
        rolling_averages[50] > rolling_averages[150]
    ):
        score += 1
